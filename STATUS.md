...
**Last updated:** 2026-04-06 (contract and proof honesty pass)  
...
| Build | Canonical local proof is `bash scripts/verify-build.sh`, which writes build, CLI smoke, test, and summary evidence to `local/verify/latest/`. Canonical shared proof is the artifact published by `.github/workflows/ci.yml`. |
...
| Supported CLI tooling | `oracle mcp`, `setup`, `doctor`, `dashboard`, `status`, `version`, and `help` |
| Dashboard scope | `oracle dashboard` prints a non-interactive terminal dashboard snapshot; it is part of the CLI surface, not a fourth product surface |
| Reasoning backends | Execution is local on macOS; reasoning may use optional configured local or remote OpenAI-compatible backends |
